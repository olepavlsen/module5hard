...
class UrTube:
    ...
    def watch_video(self, movie):
        ...
        if self.current_user is None:
            ...
            for video in self.videos:
                if video.title == self.movie and video.adult_mode != True:
                    self.scale = video.duration
                    self.time_ = video.time_now
                    for j in range(self.scale):
                        self.time_ = j + 1
                        print(self.time_, end=" ")
                        time.sleep(1)
                    print(f'Конец видео')
                elif video.title == self.movie and video.adult_mode == True:
                    self.scale = video.duration
                    self.time_ = video.time_now
